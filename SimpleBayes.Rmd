...
ggplot(draws_df_sens) +
  ...
  xlab("Predicted skied runs out of 10000") +
  ylab("Posterior Density") +
  theme_classic()
```
...
for (biasLvl in unique(sim_wide$bias_4)) {
  ...
  temp_param <- tibble(biasEstScaled = inv_logit_scaled(draws_df_param_recov$`bias[4]`), 
                   biasTrue = biasLvl, 
                   biasEst = draws_df_param_recov$`bias[4]`)
...
```{r}
# reovery plot
ggplot(recovery_df_parameter_recovery, aes(biasTrue1, biasEstScaled1)) +
