...
dataSimpleBayes <- list(
  ...
  Source1 = as.matrix(sim_wide[,12:21]),
  Source2 = as.matrix(sim_wide[,22:31]),
  prior_mean = priors$prior_mean,  # adding the created priors
  prior_sd = priors$prior_sd  # adding the created priors
)
...
sim_d_and_fit <- function(prior_mean, prior_sd) {
    ...
    draws_df_sens <- as_draws_df(samples$draws()) 
      temp <- tibble(bias_prior = draws_df_sens$`bias_prior[1]`, 
                     bias_posterior = draws_df_sens$`bias_posterior[1]`, 
                     prior_preds = draws_df_sens$`prior_preds[1]`, 
                     posterior_preds = draws_df_sens$`posterior_preds[1]`, 
                     prior_mean = prior_mean,
                     prior_sd = prior_sd)
