...
# pivot data wider
sim_wide_param <- sim_df %>% 
  group_by(skier) %>% 
  subset(select = c(skier, bias, outcome, Source1, Source2)) %>%
  mutate(row = row_number()) %>% 
  pivot_wider(names_from = skier, values_from = c(bias, outcome, Source1, Source2))

# Create the data
  dataParameterRecovery <- list(
  N = nrow(sim_wide),
  S = nskiers,
  bias = as.matrix(sim_wide_param[,2:11]),
  outcome = as.matrix(sim_wide_param[,12:21]),
  Source1 = as.matrix(sim_wide_param[,22:31]),
  Source2 = as.matrix(sim_wide_param[,32:41])
)
...
for (biasLvl in unique(sim_df$bias)) {
   ...
   param_data <- sim_df %>% subset(
      bias == biasLvl
    )
   
  samples <- mod_simpleBayes$sample(
      data = dataParameterRecovery,
      seed = 123,
      chains = 1,
      parallel_chains = 1,
      threads_per_chain = 1,
  ...
  temp_param <- tibble(biasEstScaled = inv_logit_scaled(draws_df_param_recov$bias), 
                   biasTrue = biasLvl, biasEst = draws_df_param_recov$bias)
