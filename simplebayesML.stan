...
generated quantities{
  array[S, N] int prior_preds; // distribution of skiing/non-skiing choices according to the prior
  array[S, N] int posterior_preds; // distribution of skiing/non-skiing choices according to the posterior
  ...
  for (s in 1:S) {
    ...
    for (n in 1:N) {
      prior_preds[s, n] = binomial_rng(1, bias_prior[s]);
      posterior_preds[s, n] = binomial_rng(1, inv_logit(bias[s]));
      
      log_lik[s, n] = bernoulli_logit_lpmf(outcome[n, s] | bias[s] + l_Source1[n, s] + l_Source2[n, s]);
    }
  }
}
